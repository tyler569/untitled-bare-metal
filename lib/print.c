...
get_number (struct format_spec *spec, bool signed_, va_list *args)
{
...
constexpr int NUM_MAX_DIGITS = 64;
constexpr int NUM_BUF_SIZE = NUM_MAX_DIGITS + 1;

MUST_USE
static const char *
format_layout_int (struct format_spec *spec, struct number number,
                   char *buffer)
{
...
int
vsnprintf (char *buffer, size_t n, const char *format, va_list args)
{
  struct stream f = sprintf_stream (buffer);
...
int
fnprintf (FILE *f, size_t n, const char *fmt, ...)
{
  va_list args;
  va_start (args, fmt);
