  ...
  def kernel_unmarshal_check_parameters_length
    ...
    do {
      ...
      if (len < #{mr_parameters.length})
        return msg_truncated_message(len, #{mr_parameters.length});
    } while (0);
    EOF
    ...
    do {
      ...
      if (len < #{cap_parameters.length})
        return msg_truncated_message(len, #{cap_parameters.length});
    } while (0);
    EOF
    checks = []
  ...
  def kernel_unmarshal_and_call
    ...
    case #{identifier_name}: {
      ...
      #{cap_parameters.map.with_index do |param, i|
        <<~EOS
        TRY (lookup_cap_slot (&this_tcb->cspace_root, get_cap (#{i}), 64, &#{param['name']}));
        EOS
      end.join}
...
File.open("include/#{HEADERS[:syscall_dispatch]}", 'w') do |f|
  ...
  f.puts "default:"
  f.puts "  return msg_illegal_operation();"
  f.puts "}"
  f.puts "}"
end
