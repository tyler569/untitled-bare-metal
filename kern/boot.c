...
void
create_boot_tcb (const void *initrd, size_t initrd_size)
{
  const struct elf_ehdr *init_elf = find_tar_entry (initrd, "userland");
  assert (init_elf && is_elf (init_elf));

  init_tcb_from_elf_in_this_vm (&boot_tcb, init_elf);
  strcpy (boot_tcb.name, "bootstrap");
  ...
  set_frame_r15 (&boot_tcb.saved_state, VA_IPC_BUFFER);
  set_frame_arg (&boot_tcb.saved_state, 0, VA_BOOTINFO);

  // stack: N pages contiguous VA, any PA
  for (size_t i = 0; i < STACK_PAGES; i++)
    alloc_and_map_page (init_pml4, VA_STACK + PAGE_SIZE * i, USER_RW);
  set_frame_sp (&boot_tcb.saved_state, VA_STACK_TOP);
  ...
  boot_tcb.ipc_buffer = (struct ipc_buffer *)direct_map_of (ipc_buf);

  // caps
  cap_t boot_tcb_cap = new_tcb_cap (&boot_tcb);
  ...
  struct
  {
    ...
    { INIT_CAP_INIT_TCB, boot_tcb_cap },
  ...
  boot_tcb.cspace_root.cap = init_cnode_cap;
  boot_tcb.vspace_root.cap = init_vspace_cap;
