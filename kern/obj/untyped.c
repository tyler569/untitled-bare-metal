...
error_t
untyped_retype (cap_t *untyped, word_t type, word_t size_bits, cap_t root,
                word_t index, uint8_t depth, word_t offset, word_t num_objects)
{
  ...
  cap_t dest_cnode;
  error_t err = lookup_cap (root, index, depth, &dest_cnode);
  if (err != no_error)
    {
      return_ipc (err, 0);
      return -1;
  ...
  if (cap_size (dest_cnode) < offset + num_objects)
    {
      return_ipc (range_error, 0);
      return -1;
    }

  cte_t *dest_slot_0 = cte_for (dest_cnode, offset, depth);
